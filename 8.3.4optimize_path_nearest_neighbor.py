...
# ================== CONFIG ==================
IMAGE_PATH = '/Users/student/Downloads/_.jpeg'
...
DOBOT_SPEED = 500        # เพิ่มความเร็ว (ค่าสูงสุดที่ Dobot Magician รับได้)
DOBOT_ACCELERATION = 250 # เพิ่มอัตราเร่ง (ช่วยให้เคลื่อนที่เร็วขึ้น)
...
# ----------------- ❗️❗️❗️ [NEW] ฟังก์ชันใหม่ 2 ตัว -----------------

def optimize_path_nearest_neighbor(all_contours_data, start_original_index):
    """
    จัดเรียงลำดับ Contours ใหม่โดยใช้หลักการ Nearest Neighbor
    เพื่อหาเส้นทางที่ "ใกล้ที่สุด" ถัดไปเสมอ
    """
    if not all_contours_data:
        return []

    optimized_order = []
    remaining_paths = list(all_contours_data)
    
    # 1. หา Contour เริ่มต้นตามที่ผู้ใช้เลือก
    start_path = None
    for path in remaining_paths:
        if path['original_index'] == start_original_index:
            start_path = path
            break
    
    # (กรณีหาไม่เจอ หรือผู้ใช้ไม่ได้เลือก)
    if start_path is None:
        start_path = remaining_paths[0] 

    optimized_order.append(start_path)
    remaining_paths.remove(start_path)
    
    # 2. หาจุดสิ้นสุด (mm) ของ Contour ปัจจุบัน
    last_point_mm = start_path['end_point_mm']

    # 3. วน Loop จนกว่าจะจัดลำดับครบ
    while remaining_paths:
        min_dist = float('inf')
        best_path = None
        
        # 4. ค้นหาในเส้นที่เหลือทั้งหมด ว่าเส้นไหน "ใกล้" ที่สุด
        for next_path in remaining_paths:
            start_point_mm = next_path['start_point_mm']
            dist = math.dist(last_point_mm, start_point_mm)
            
            if dist < min_dist:
                min_dist = dist
                best_path = next_path
        
        # 5. เพิ่มเส้นที่ "ดีที่สุด" ลงใน List
        optimized_order.append(best_path)
        remaining_paths.remove(best_path)
        last_point_mm = best_path['end_point_mm']

    return optimized_order

def visualize_optimized_path(base_img_gray, optimized_data, M_inv, output_dir):
    """
    ...
    try:
        ...
        all_contours_px = [item['contour_px'] for item in optimized_data]
        cv2.drawContours(vis_img, all_contours_px, -1, (255, 0, 0), 1) # (Blue)

        # 2. วาดเส้นเดินทาง (สีแดง)
        for i in range(len(optimized_data) - 1):
            # 2a. หาจุดสิ้นสุด (mm) ของเส้นนี้ และ จุดเริ่มต้น (mm) ของเส้นถัดไป
            end_point_mm = optimized_data[i]['end_point_mm']
            start_point_mm = optimized_data[i+1]['start_point_mm']
        ...
        # 3. บันทึกภาพ
        save_path = os.path.join(output_dir, "05_optimized_path_visualization.jpg")
        cv2.imwrite(save_path, vis_img)
        print(f"✅ บันทึกภาพ '05_optimized_path_visualization.jpg' สำเร็จ!")
...
def run_drawing_mode(bot): 
    ...
    M = cv2.getPerspectiveTransform(img_corners, PAPER_CORNERS)
    # ❗️❗️❗️ [NEW] - คำนวณ Matrix (M_inv) สำหรับแปลง mm กลับไปเป็น px
    ...
    # ❗️❗️❗️ [MODIFIED] - เปลี่ยนวิธีเก็บข้อมูลเป็น List of Dictionaries ❗️❗️❗️
    all_contours_data = [] # นี่คือ List ที่จะเก็บข้อมูลทั้งหมด
    total_drawing_length_check = 0 # (ใช้ total_drawing_length จาก process_and_draw_contours)
    
    for i, cnt_px in enumerate(filtered_contours):
        if len(cnt_px) < 2:
            continue
            
        # (เราใช้ cnt_px (คือ approx) ที่ได้จาก filtered_contours อยู่แล้ว)
        ...
        # เก็บข้อมูลทั้งหมดไว้ใน Dictionary
        all_contours_data.append({
            'original_index': i,                 # Index ดั้งเดิม (0, 1, 2, ...)
            'path_mm': pts_transformed_mm,       # พิกัด (mm) สำหรับ Dobot
            'contour_px': cnt_px,                # พิกัด (px) สำหรับวาดภาพ
            'length_mm': length_mm,              # ความยาว (mm)
            'start_point_mm': pts_transformed_mm[0][0],  # จุดเริ่ม (mm)
            'end_point_mm': pts_transformed_mm[-1][0]    # จุดจบ (mm)
        })
        
        total_drawing_length_check += length_mm

    print(f"✅ ตรวจสอบความยาวรวม: {total_drawing_length_check:.2f} mm")
    ...
    while True: 
        ...
        if method_choice == '1':
            while True:
                try:
                    user_input = input(f"👉 กรุณาใส่หมายเลข Contour เริ่มต้น (1 ถึง {len(all_contours_data)}) หรือ Enter เพื่อใช้ 1: ")
                    if not user_input:
                        START_CONTOUR_INDEX = 1
                    else:
                        START_CONTOUR_INDEX = int(user_input)
                        if not (1 <= START_CONTOUR_INDEX <= len(all_contours_data)):
                            print(f"⚠️ หมายเลขต้องอยู่ระหว่าง 1 ถึง {len(all_contours_data)}")
                            continue
            ...
            try:
                ...
                for i, data in enumerate(all_contours_data):
                    start_point_x, start_point_y = data['start_point_mm']
    ...
    # ❗️❗️❗️ [NEW] - เรียกใช้ Path Optimization ❗️❗️❗️
    print(f"⏳ กำลังเพิ่มประสิทธิภาพเส้นทาง (Path Optimization) โดยเริ่มจากเส้นที่ {START_CONTOUR_INDEX}...")
    optimized_data = optimize_path_nearest_neighbor(all_contours_data, start_index_original)
    print("✅ เพิ่มประสิทธิภาพเส้นทางเสร็จสิ้น!")

    # ❗️❗️❗️ [NEW] - แตกข้อมูลที่จัดลำดับใหม่ กลับไปเป็น List ❗️❗️❗️
    # (เราทำเช่นนี้เพื่อให้โค้ดส่วนที่เหลือ (การวาด, การสร้าง checkpoint) ทำงานได้เหมือนเดิม)
    processed_paths = [item['path_mm'] for item in optimized_data]
    filtered_contours = [item['contour_px'] for item in optimized_data] # ⭐️ นี่คือ List contour (px) ที่จัดลำดับใหม่
    contour_lengths = [item['length_mm'] for item in optimized_data]
    original_indices = [item['original_index'] + 1 for item in optimized_data] # ( +1 เพื่อให้เริ่มนับจาก 1)
    
    # ❗️❗️❗️ [DELETE] - ลบตรรกะ "Wrap-around" (จัดลำดับ List) แบบเก่าทิ้ง ❗️❗️❗️
    # (ส่วนนี้ถูกแทนที่ด้วย optimize_path_nearest_neighbor แล้ว)

    # (ส่วนการสร้าง Checkpoint จะทำงานได้ทันที
    #  เพราะ 'filtered_contours' ถูกจัดลำดับใหม่ (Optimize) แล้ว)
    print(f"⏳ กำลังสร้างภาพลำดับการวาด (ที่ Optimize แล้ว) ลงใน {OUTPUT_ALL_STEPS_PATH}/ ...")
    ...
    # ❗️❗️❗️ [NEW] - สร้างภาพ Visualization (เส้นสีแดง) ❗️❗️❗️
    print(f"⏳ กำลังสร้างภาพแสดงเส้นทางที่ Optimize แล้ว (05_optimized_path_visualization.jpg)...")
    visualize_optimized_path(img, optimized_data, M_inv, OUTPUT_CURRENT_RUN_PATH)
    
    
    print("\n" + "="*50)
    print(f"⭐️ คุณสามารถตรวจสอบ 'ลำดับการวาด' ทั้งหมดได้แล้ว")
    print(f"   ในโฟลเดอร์: {OUTPUT_ALL_STEPS_PATH}/")
    print(f"⭐️ และดู 'เส้นทางที่ Optimize' (เส้นสีแดง) ได้ที่:")
    print(f"   ในโฟลเดอร์: {os.path.join(OUTPUT_CURRENT_RUN_PATH, '05_optimized_path_visualization.jpg')}")
    print("="*50 + "\n")
    ...
    # (แสดง Log เริ่มต้น)
    # ❗️❗️❗️ [MODIFIED] - อัปเดต Log ให้ใช้ original_indices[0] ❗️❗️❗️
    print(f"▶️ เริ่มวาดจาก contour ดั้งเดิมที่ {original_indices[0]} / {len(processed_paths)} (ความยาวรวมที่ต้องวาด: {total_length_to_draw:.2f} mm)")
    ...
    for i in range(len(processed_paths)):
        ...
        ci_original = original_indices[i] # (นี่คือหมายเลข Contour ดั้งเดิม (มี +1 แล้ว))
