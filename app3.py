...
import os
import matplotlib
matplotlib.use('Agg') # ตั้งค่า Matplotlib ไม่ให้ใช้ GUI
import matplotlib.pyplot as plt
import cv2  
import threading # สำหรับการวาดแบบไม่บล็อก Server
...
try:
    ...
    # สำหรับเวอร์ชันเก่าที่ไม่มี Image.Resampling
...
g_generated_paths = {
    ...
    'bg_img': None,
    'm_inv': None,
    'image_dims': (400, 400) # (w, h) ของภาพที่ถูกปรับขนาด
}
# ==============================================================================
...
# ⭐️ [NEW] แก้ไขฟังก์ชันนี้เพื่อรองรับการวาดเส้นบางส่วน (Live Progress)
def create_progress_image(base_img_bgr, sorted_paths_mm, current_set_index, M_inv, path_prefix, is_final=False, current_point_index=None):
    """สร้างและบันทึกภาพ Progress (CV2) โดยใช้ M_inv ในการแปลงพิกัด mm -> px"""
    ...
    try:
        ...
        # เส้นวาดปัจจุบัน (สีเขียว หรือ สีแดง หากวาดบางส่วน)
        if not is_final and current_set_index < len(sorted_paths_mm):
            path_mm_current = sorted_paths_mm[current_set_index]
            
            # ⭐️ [NEW] Logic ใหม่สำหรับ Live Progress
            path_to_draw = path_mm_current
            color = (0, 255, 0) # สีเขียว (Default - เส้นถัดไป)
            thickness = 3

            # ถ้ามีการส่ง current_point_index มา (แปลว่ากำลังวาดเส้นนี้อยู่)
            if current_point_index is not None and current_point_index > 1:
                path_to_draw = path_mm_current[:current_point_index] # เลือกเฉพาะจุดที่วาดไปแล้ว
                color = (0, 0, 255) # สีแดง (กำลังวาด)
                thickness = 2
            
            # วาดเส้น (อาจจะเป็นเส้นเต็ม หรือเส้นบางส่วน)
            path_px = cv2.perspectiveTransform(path_to_draw, M_inv).reshape(-1, 2)
            cv2.polylines(preview, [path_px.astype(np.int32)], isClosed=False, color=color, thickness=thickness) 
            
            # ⭐️ [NEW] ถ้ากำลังวาด Live (สีแดง) ให้วาดเส้นที่เหลือเป็นสีเขียวจางๆ
            if current_point_index is not None and current_point_index > 1 and current_point_index < len(path_mm_current):
                path_remaining = path_mm_current[current_point_index-1:] # -1 เพื่อให้เส้นต่อกัน
                path_px_remaining = cv2.perspectiveTransform(path_remaining, M_inv).reshape(-1, 2)
                cv2.polylines(preview, [path_px_remaining.astype(np.int32)], isClosed=False, color=(150, 255, 150), thickness=1) # สีเขียวจาง

        # บันทึกทับไฟล์เดิมเสมอ
        filename_current = os.path.join(path_prefix, f"current_progress.png")
        
        # ⭐️ [FIX] เพิ่ม try-except ป้องกันไฟล์ 0-byte
        try:
            cv2.imwrite(filename_current, preview)
        except Exception as e_write:
            print(f"⚠️ [ERROR] ไม่สามารถบันทึก Progress Image ได้: {e_write}")

        
        # อัปเดต Global State ให้หน้าเว็บดึงไปแสดงผล
        if g_drawing_state['status'] != 'idle':
            g_drawing_state['progress_image_url'] = f"/output/current_progress.png?t={time.time()}"
            
    except Exception as e_main:
        # ดักจับข้อผิดพลาดอื่นๆ ที่อาจเกิดขึ้นในฟังก์ชันนี้
        print(f"⚠️ [ERROR] เกิดปัญหาใน create_progress_image: {e_main}")
...
# --- (ฟังก์ชัน sort_contours_for_efficiency) ---
...
# --- (ฟังก์ชัน filter_overlapping_contours) ---
...
def run_experiment(exp_name, contour_approx_epsilon, max_connect_distance, base_paths_px, base_img_bgr, paper_corners_mm):
    ...
    if num_sets > 0:
        ...
        # สร้างภาพ Progress ภาพสุดท้าย (ภาพที่เสร็จสมบูรณ์)
        # ⭐️ [FIX] ส่งพารามิเตอร์ให้ครบ (current_point_index=None เป็นค่า default อยู่แล้ว)
...
# ==============================================================================
# ⬇️⬇️⬇️ ฟังก์ชันสำหรับ Thread การวาด (⭐️ แก้ไขเพื่อ Live Progress) ⬇️⬇️⬇️
# ==============================================================================
...
def drawing_thread_function():
    ...
    # ⭐️ [NEW] ตั้งค่าการอัปเดต Live Progress (ทุก 5 วินาที)
    LIVE_PROGRESS_UPDATE_INTERVAL = 5.0 # [วินาที]
    ...
    try:
        ...
        # บันทึกภาพสถานะเริ่มต้น (Contour Set 0)
        ...
        # ⭐️ [NEW] ตัวแปรสำหรับจับเวลา Live Progress
        last_progress_image_time = time.time()

        for i in range(start_index, len(sorted_paths_final_mm)):
            
            # ตรวจสอบ Flag การหยุด/Pause
            ...
            # สร้างภาพ Progress ก่อนวาดเส้นนี้ (แสดงเป็นสีเขียว)
            create_progress_image(preview_img_bgr, sorted_paths_final_mm, i, M_inv, OUTPUT_CURRENT_RUN_PATH, is_final=False, current_point_index=None)
            ...
            # 3. วาด (⭐️ [NEW] แก้ไข Loop เพื่อส่ง Index)
            #    (เราเริ่มที่ 1 เพราะจุดที่ 0 คือจุดเริ่มต้น sx, sy ที่วาดไปแล้ว)
            for point_idx, p in enumerate(pts_transformed[1:], start=1):
                # ตรวจสอบ Flag การหยุด/Pause ภายใน Loop
                if g_stop_drawing_flag or g_drawing_state['status'] == 'paused':
                    break 
                ...
                # ⭐️ [NEW] Logic การอัปเดต Live Progress ทุก 5 วินาที
                current_time = time.time()
                if current_time - last_progress_image_time > LIVE_PROGRESS_UPDATE_INTERVAL:
                    # อัปเดตภาพ Progress (โดยส่ง point_idx + 1 เพราะ enumerate เริ่มที่ 1 และเราต้องการรวมจุดปัจจุบัน)
                    create_progress_image(preview_img_bgr, sorted_paths_final_mm, i, M_inv, OUTPUT_CURRENT_RUN_PATH, is_final=False, current_point_index=(point_idx + 1))
                    last_progress_image_time = current_time # รีเซ็ตเวลา
            
            # ตรวจสอบอีกครั้งหลัง Loop เผื่อโดนสั่งหยุด
            if g_stop_drawing_flag:
                g_drawing_state['message'] = 'Stopped by user'
                break
...
def process_image_api():
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ [ERROR] ไม่สามารถบันทึก Canny Image ได้: {e_write}")
            return jsonify({'status': 'error', 'message': f'Failed to save Canny image: {e_write}'})
...
def serve_output_file(filename):
    """API: เสิร์ฟไฟล์ภาพที่ประมวลผล (Canny, Plan, Progress)"""
    response = send_from_directory(OUTPUT_CURRENT_RUN_PATH, filename)
    # เพิ่ม Cache Control เพื่อให้รูป Progress โหลดใหม่ได้
    response.headers['Cache-Control'] = 'no-cache, no-store, must-revalidate'
    response.headers['Pragma'] = 'no-cache'
    response.headers['Expires'] = '0'
    return response

# ⭐️ โค้ด Frontend คาดหวังการเรียก API '/get_path_data' สำหรับ Preview Animation 
...
def get_progress_api():
    """API: สำหรับหน้าเว็บ Polling สถานะ (พร้อม Progress Image URL)"""
...
@app.route('/pause_drawing', methods=['POST']) # รองรับทั้ง /pause และ /pause_drawing
...
@app.route('/resume_drawing', methods=['POST']) # รองรับทั้ง /resume และ /resume_drawing
...
@app.route('/stop_drawing', methods=['POST']) # รองรับทั้ง /stop และ /stop_drawing
def stop_drawing_api():
    ...
    if g_drawing_state['status'] in ['drawing', 'paused']:
        ...
        try:
            if g_bot:
                # สั่งหยุดการทำงานที่ค้างอยู่ในคิวทันที
...
def get_initial_status_api():
    """API สำหรับ Frontend เรียกเมื่อโหลดหน้าเพื่อตรวจสอบสถานะการเชื่อมต่อเริ่มต้น"""
    global g_bot
...
if __name__ == "__main__":
    ...
    print("    Access at: http://127.0.0.1:5012") # ⭐️ เปลี่ยน Port กลับเป็น 5012 ตามโค้ดเดิม
    print("============================================")
    app.run(debug=True, host='0.0.0.0', port=5017, use_reloader=False)